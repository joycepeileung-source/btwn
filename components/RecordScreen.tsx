...
const RecordScreen: React.FC<RecordScreenProps> = ({ onCancel }) => {
  ...
  // Helper to detect best MIME type for the device
  const getSupportedMimeType = () => {
    const types = [
      'audio/mp4', // Preferred for iOS
      'audio/aac',
      'audio/webm;codecs=opus', // Preferred for Chrome/Android
      'audio/webm',
  ...
  // Helper to convert Blob to Base64 for permanent storage
  ...
  const togglePlayback = (e: React.MouseEvent) => {
    ...
    if (isPlayingBack) {
      ...
      if (playbackRef.current.ended) playbackRef.current.currentTime = 0;
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          console.warn("Reverse geocoding failed. Falling back to raw coords:", error);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Convert Blob to persistent Base64 string
      if (recordedBlob) {
        finalAudioUrl = await blobToBase64(recordedBlob);
      }
            ...
            >
              ...
              <div className="mt-2">
                <motion.button
                  ...
                  onClick={handleSave}
                  disabled={isSaving}
                  className={`w-full py-5 bg-terracotta text-white rounded-[24px] text-[16px] font-medium tracking-wide shadow-xl shadow-terracotta/20 flex items-center justify-center gap-3 ${isSaving ? 'opacity-70' : ''}`}
                >
                  {isSaving ? 'Archiving...' : 'Add to My Journals'}
                </motion.button>
              </div>
            </motion.div>
          </div>
