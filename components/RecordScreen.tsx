...
const RecordScreen: React.FC<RecordScreenProps> = ({ onCancel }) => {
  ...
  // Helper to detect best MIME type for the device (Crucial for iOS/Safari)
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = () => {
        // Use the mimeType from the recorder to ensure the Blob is valid
  ...
  const togglePlayback = (e: React.MouseEvent) => {
    ...
    // iOS requires Audio elements to be created/interacted within a click event
    ...
    if (isPlayingBack) {
      ...
      playbackRef.current.play()
        ...
        .catch(err => {
          ...
          alert("Unable to play recording. This can happen if the device is in silent mode or if the file format is unsupported.");
        });
    }
  };
