...
const JournalArchive: React.FC<JournalArchiveProps> = ({ onBack }) => {
  ...
  const togglePlayback = (journal: SavedRecording, e: React.MouseEvent) => {
    ...
    const urlToPlay = journal.audioUrl || AMBIENT_SOUNDS[Math.floor(Math.random() * AMBIENT_SOUNDS.length)].url;
    
    const audio = new Audio(urlToPlay);
    audio.onended = () => setPlayingJournalId(null);
    audio.play().catch(err => {
      console.warn("Playback failed:", err);
      audio.src = AMBIENT_SOUNDS[0].url;
      audio.play();
    });
  ...
  const detectLocation = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          // Browser-safe fetch-based reverse geocoding
